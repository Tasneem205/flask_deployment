...
imgmodel_path = os.path.join(directory, "igmodell.h5")
...
def predict_scan():
    ...
    if file and allowed_file(file.filename):
        ...
        prediction = imgModel.predict(scan_image)
        predicted_class_index = np.argmax(prediction)
        class_labels = {
            0: "Cyst",
            1: "Normal",
            2: "Stone",
            3: "Tumor"  # Adjust according to your actual class labels
        }
        predicted_class_label = class_labels[predicted_class_index]
        return jsonify({"message": "Image uploaded successfully", "prediction": predicted_class_label}), 200
        return jsonify({"message": "Image code run"}), 200
    else:
